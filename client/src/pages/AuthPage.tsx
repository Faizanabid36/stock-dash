...
const Auth = () => {
  const [getOwnedStocks, { data: ownedStocksData }] =
  ...
  useEffect(() => {
    document.title = 'Authorization | StockDash';
  }, []);
      ...
      loginMutation({
        ...
        .then(({ data }) => {
          ...
          if (ownedStocksData) {
            dispatch({
              type: OWNED_STOCKS,
              payload: ownedStocksData?.ownedStocks,
            });
          }
        ...
        {!auth?.user ? (
          <>
            ...
            <div className='flex items-center justify-center py-4 text-center bg-gray-100 dark:bg-gray-800'>
              ...
              <button
                onClick={switchMode}
                className='mx-2 text-sm font-bold text-pink-600 dark:text-pink-600 hover:text-pink-800'
              >
