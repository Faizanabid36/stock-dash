...
const AccountPage = () => {
  ...
  useEffect(() => {
    if (auth) {
      document.title = auth?.user?.username + ' | StockDash';
    }
  }, [auth]);
      ...
      depositMutation({ variables: { amount: transferAmount } })
        .then(({ data }: any) => {
          setTimeout(() => {
            ...
            dispatch({
              type: UPDATE_BALANCE,
              payload: { newBalance: data?.deposit.newBalance },
            });
      ...
      withdrawMutation({ variables: { amount: transferAmount } })
        .then(({ data }) => {
          setTimeout(() => {
            ...
            dispatch({
              type: UPDATE_BALANCE,
              payload: { newBalance: data?.withdraw?.newBalance },
            });
            setIsLoadingWithdraw(false);
            setTransferAmount(0);
          }, 1500);
      ...
      .then(({ data }) => {
        setTimeout(() => {
          ...
          dispatch({
            ...
            payload: { newUsername: data?.changeUsername.newUsername },
          });
      ...
      {auth && (
        <div className='dark:bg-darkBg flex flex-col min-h-screen items-center justify-center overflow-hidden'>
          <div className='container flex flex-col md:items-center text-gray-900 dark:text-white'>
            <h1 className='title-font text-center sm:text-2xl text-xl mb-8 font-medium'>
              My Account
            </h1>
            <div className='md:flex justify-center items-start h-full md:w-3/4 md:h-3/4'>
              <Tab.Group vertical>
                ...
                <Tab.Panels className='dark:bg-darkField bg-gray-100 rounded-2xl dark:text-white p-3 min-h-panel w-full'>
                  <Tab.Panel
                    key={'Profile'}
                    className={classNames('min-h-panel')}
                  >
                      ...
                      <section className='mx-auto pb-7 divide divide-y w-full flex-1'>
                        ...
                        <div className='h-full pt-3'>
                          <div className='flex flex-col h-full space-y-4 justify-evenly item-center'>
                            ...
                            <h5 className='text-center text-sm font-light'>
                          ...
                          <div className='px-4 py-2 -mx-3'>
                            <div className='mx-3'>
                              <span className='font-semibold text-red-700 dark:text-red-700'>
                                <span className='text-sm text-red-700 dark:text-red-700'>
                                  {errors}
                                </span>
                              </span>
                            </div>
